...
async function runNext() {
  ...
  try {
    await ingestCompany({ searchTerm: job.cik, forms: job.forms, onProgress: (progress) => update(job, { status: progress.status, progress }) });
    clearAnswerCache();
...
export async function requestIngestion({ company, forms = ["10-K", "10-Q"], ingest = ingestCompany } = {}) {
  const existing = [...jobs.values()].find((job) => job.cik === company.cik && ["QUEUED", "RUNNING", "DOWNLOADING", "INDEXING", "UPDATING_BM25"].includes(job.status));
...
export async function getIngestionStatus(cik, { company } = {}) {
  const active = [...jobs.values()].reverse().find((job) => job.cik === String(cik).padStart(10, "0"));
  if (active) return publicJob(active);
  if (company) {
    const indexed = await getIndexedCompanyStatus(company);
    return { cik: company.cik, company, status: indexed.indexed ? "READY" : "NOT_REQUESTED", progress: null };
  }
  return { cik: String(cik).padStart(10, "0"), status: "NOT_REQUESTED", progress: null };
}
